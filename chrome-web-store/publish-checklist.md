...
- [x] 版本号：2.5.7（语义化版本；与 `manifest.json` 一致）
...
### 8. 打包准备 ✅
- [x] 清理发布构建目录中的临时文件
- [x] 压缩为 ZIP：已生成 `dist/page-scroll-master-v2.5.7.zip`
- [x] ZIP 包大小 < 2MB（220,662 bytes / 215.49 KB / 0.21 MB，46 个条目）
- [x] 所有必要文件都包含
- [x] 无多余文件
- [x] 文件结构正确，`unzip -t` 通过
...
2. **版本号策略**：以 `manifest.json` 中的版本号为准，v2.5.7 发布目标为 2.5.7
3. **描述优化**：使用关键词，提高搜索排名
...
**最后更新时间**：2026-08-21
**状态**：v2.5.7 发布材料和发布 ZIP 已准备完成，待提交商店后台
**构建产物**：`dist/page-scroll-master-v2.5.7.zip`（220,662 bytes / 215.49 KB / 0.21 MB，46 个条目）
**SHA-256**：`c24b80fd284ec82d35ff49ef4ac468b69c0a6f69d261739d350570e992259b13`
**验收说明**：v2.5.7 源码与打包后回归、商店材料校验、发布构建和 ZIP 完整性验证已完成；正式 ZIP 的飞书/Lark 长表手工验收和商店后台填写仍待人工完成
