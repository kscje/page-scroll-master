...
- [x] 版本号：2.4.0（语义化版本；与 `manifest.json` 一致）
...
### 8. 打包准备 ✅
- [x] 清理发布构建目录中的临时文件
- [ ] 压缩为 ZIP：待用户明确要求后生成 `dist/page-scroll-master-v2.4.0.zip`
- [ ] ZIP 包大小 < 2MB
- [ ] 所有必要文件都包含
- [ ] 无多余文件
- [ ] 文件结构正确，`unzip -t` 通过
...
2. **版本号策略**：以 `manifest.json` 中的版本号为准，v2.4 发布目标为 2.4.0
3. **描述优化**：使用关键词，提高搜索排名
...
**最后更新时间**：2026-06-16
**状态**：v2.4.0 发布材料已同步，发布 ZIP 待用户明确要求后生成
**构建产物**：待生成 `dist/page-scroll-master-v2.4.0.zip`
**验收说明**：v2.4 真实 Chrome 视觉验收和商店后台填写仍待人工完成
