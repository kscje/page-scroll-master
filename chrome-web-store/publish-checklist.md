...
- [x] 版本号：2.5.2（语义化版本；与 `manifest.json` 一致）
...
### 8. 打包准备 ✅
- [x] 清理发布构建目录中的临时文件
- [x] 压缩为 ZIP：已生成 `dist/page-scroll-master-v2.5.2.zip`
...
2. **版本号策略**：以 `manifest.json` 中的版本号为准，v2.5.2 发布目标为 2.5.2
3. **描述优化**：使用关键词，提高搜索排名
...
**最后更新时间**：2026-07-05
**状态**：v2.5.2 发布材料和发布 ZIP 已准备完成，待提交商店后台
**构建产物**：`dist/page-scroll-master-v2.5.2.zip`（201.71 KB / 0.20 MB）
**验收说明**：v2.5.2 源码回归、商店材料校验和发布构建已完成；商店后台填写仍待人工完成
