...
- [ ] 页面目录、上一段、下一段和当前章节高亮
- [ ] SPA 路由变化、动态内容变化和目录设置变化后旧目录失效并可重建
...
# 使用统一构建脚本创建 ZIP 包，避免把开发工具、文档和系统文件放入商店包
node build.js
```

发布 ZIP 输出到 `dist/page-scroll-master-v<manifest version>.zip`，上传前应检查文件名、内容和大小。
...
商店后台所需文案、隐私政策、图标、宣传图和多语言截图统一维护在仓库根目录的 `chrome-web-store/`。提交前先阅读 `chrome-web-store/README.md`，并运行：
...
- 支持主题自适应
- 动态图标更新
- 智能段落跳转只在当前页面内存中解析目录，不上传页面标题或页面结构
