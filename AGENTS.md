...
- 撰写 `doc/CHANGELOG.md` 和设置页更新记录时，`Added` / “新功能”只用于全新用户能力、全新功能模块、全新语言、全新服务入口或重大功能面扩展。
- 设置页结构调整、折叠展示、恢复默认入口、默认值调整、布局优化、兼容性增强、性能改善、稳定性增强和测试补强，应归入 `Changed` / “功能优化”，即使文案中出现“新增入口”或“新增控制项”。
- 明确修复错误行为、回归、误选滚动容器、状态异常或兼容问题时，归入 `Fixed` / “Bug 修复”。
- 同一版本的 `doc/CHANGELOG.md` 与 `options.js` 本地更新记录分类应保持一致；面向用户的设置页更新记录应简短说明用户可感知变化，不展开内部实现细节。
