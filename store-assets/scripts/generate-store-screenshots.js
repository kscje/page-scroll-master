...
function settingsMockup(x, y, locale) {
  ...
  return `
    <g filter="url(#softShadow)">
      ...
      ${text({ x: x + 78, y: y + 44, value: chinese ? 'Page Scroll Master 设置' : 'Page Scroll Master Settings', size: 24, weight: 760, fill: '#23384c' })}
      ${labels.map((label, i) => {
        const yy = y + 116 + i * 76;
        return `
...
const assets = [
  {
    ...
    svg: frame(`
      ...
      ${text({ x: 184, y: 118, value: 'Page Scroll Master', size: 52, weight: 850, fill: '#18324a' })}
