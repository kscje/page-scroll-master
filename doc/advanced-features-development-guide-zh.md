...
### v3.0 Direct Feedback Service Update

v3.0 聚焦扩展内直接提交建议和问题反馈，在用户量达到需要系统化收集反馈的阶段再开发：

- 设置页新增可直接提交的“建议&反馈”表单
- 用户点击确认后反馈提交到服务端，并转发到开发者邮箱
- 支持用户主动上传图片
- 页面 URL 默认不附带，用户勾选后尽力读取并随反馈提交
...
v3.0 推荐采用方案 B：Cloudflare Worker + 邮件服务。

推荐边界：

- v1.9 不实现该模块，等用户量达到需要系统化收集反馈的阶段再进入 v3.0。
- 反馈服务端首选 Cloudflare Worker，邮件转发服务可选 Resend 或同类邮件 API。
- 不建设完整管理后台；v3.0 首版只做扩展表单、服务端校验、邮件转发和基础日志。
- 图片上传由用户主动选择，默认不上传任何页面截图。
- 页面 URL 默认不附带；用户勾选后尽力读取，读取失败时降级为空或 `unavailable`。
...
pageUrl: 用户勾选后尽力读取，读取失败为空或 unavailable
images[]: 用户主动选择的图片文件
```
...
- README、隐私政策和商店隐私披露必须说明反馈内容、图片、可选页面 URL 会提交到反馈服务并转发到开发者邮箱。
