...
### v1.9 Reading Position & Language Update

v1.9 聚焦阅读位置恢复和多语言覆盖扩展：
...
### v2.1 Product Experience & Privacy Update
...
整体产品规划见 `doc/v2.1-product-plan-zh.md`。匿名统计详细规格见 `doc/v2.1-usage-analytics-spec-zh.md`，实施步骤见 `doc/v2.1-usage-analytics-implementation-plan-zh.md`。

### v2.2 Navigation Shortcuts & Feedback Update

v2.2 聚焦快捷导航效率和扩展内反馈闭环：

- 支持用户查看并自定义“回到顶部”和“回到底部”的 Chrome 全局快捷键
- 新增“跳转到上一屏”和“跳转到下一屏”，兼容根页面与自定义滚动容器
- 上一屏和下一屏按钮分别支持自定义颜色，并使用区别于顶部/底部箭头的专用视口图标
- 简化滚动位置书签和智能段落跳转按钮位置：移除“上/下按钮之间”，改为页面顶部、底部或中部；中部工具排列在“回到底部”按钮之后
- 设置页新增可直接提交的“建议&反馈”表单
- 用户点击确认后反馈提交到服务端，并转发到开发者邮箱
- 支持用户主动上传图片
- 反馈表单不读取或发送页面 URL 和浏览器语言
- 更新隐私政策和 Chrome Web Store 隐私披露

整体产品规划见 `doc/v2.2-product-plan-zh.md`。

### v2.3 Auto Reading & Page Navigation Update

v2.3 聚焦连续阅读、分页内容导航和语言覆盖扩展：

- 新增自动滚屏播放，支持播放、暂停、速度调节和智能暂停
- 自动滚屏兼容根页面、自定义滚动容器、SPA 和动态内容
- 在智能段落跳转窗口内新增高置信度“上一页/下一页”导航
- 分页导航默认不创建额外浮动按钮，避免页面按钮过于繁杂
- 新增俄语、土耳其语和印度尼西亚语支持

整体产品规划见 `doc/v2.3-product-plan-zh.md`。
...
- 所有按钮透明度统一读取 `buttonSettings.opacity`；`advancedSettings` 内不再为按屏跳转维护独立透明度
...
> v2.2 位置规则修订：滚动位置书签和智能段落跳转不再提供“上/下按钮之间”，改为“页面中部”；页面中部工具排列在“回到底部”按钮之后。完整规则以 `doc/v2.2-product-plan-zh.md` 为准。
...
## v2.2 功能：服务端建议&反馈提交模块

### 产品目标

让用户可以在设置页直接提交建议、问题反馈或功能请求。用户点击确认后，扩展内显示提交结果，反馈内容由服务端转发到开发者邮箱，不依赖用户本地邮件客户端。

### 方案评估

#### 方案 A：第三方表单服务

扩展直接提交到第三方表单服务，由第三方服务将反馈转发到开发者邮箱。

优点：

- 上线速度最快。
- 不需要自建后端。
- 可利用服务商提供的文件上传、邮件通知和垃圾提交控制。

缺点：

- 依赖第三方服务稳定性和套餐限制。
- 反馈内容与图片会经过第三方服务，隐私政策需要披露。
- 后续扩展自定义限流、日志和数据处理规则不够灵活。

#### 方案 B：Cloudflare Worker + 邮件服务

扩展将表单和图片提交到 Cloudflare Worker，Worker 做基础校验、限流和反垃圾处理，再调用邮件服务将反馈内容发送到开发者邮箱。

优点：

- 用户体验符合“点击确认即提交成功”。
- 邮件服务 API key 只保存在 Worker secrets 中，不进入扩展包。
- 可自定义图片大小限制、请求频率、来源校验和错误提示。
- 后续可扩展到管理后台、标签、状态追踪或统计。

缺点：

- 需要部署和维护 Worker。
- 需要配置邮件服务域名、发件身份和 secrets。
- 扩展需要新增反馈服务域名的远程请求权限。
- 隐私政策和 Chrome Web Store 隐私实践必须更新。

### v2.2 推荐方案

v2.2 推荐采用方案 B：Cloudflare Worker + 邮件服务。

推荐边界：

- v1.9 不实现该模块，等用户量达到需要系统化收集反馈的阶段再进入 v2.2。
- 反馈服务端首选 Cloudflare Worker，邮件转发服务可选 Resend 或同类邮件 API。
- 不建设完整管理后台；v2.2 首版只做扩展表单、服务端校验、邮件转发和基础日志。
- 图片上传由用户主动选择，默认不上传任何页面截图。
- 反馈表单不读取或发送页面 URL 和浏览器语言。
...
```text
POST https://feedback.page-scroll-master.example/feedback
...
```text
type: feature | bug | compatibility | translation | other
message: 用户输入内容
contact: 用户主动填写的联系方式，可为空
...
- 不读取或发送当前页面 URL 和浏览器语言。
- 默认不附带用户邮箱，联系方式由用户主动填写。
- 默认不上传图片，图片必须由用户主动选择。
- 反馈内容不保存到扩展 storage。
- 服务端只接收反馈提交接口，不采集页面内容、滚动位置、书签 URL 或站点使用历史。
- 反馈服务需要基础限流、文件类型限制、文件大小限制和错误提示。
- README、隐私政策和商店隐私披露必须说明反馈内容和图片会提交到反馈服务并转发到开发者邮箱，且不会读取页面 URL 或浏览器语言。

## v2.0 功能：智能段落跳转

智能段落跳转的最终 v2.0 产品边界、解析规则、菜单结构、SPA 生命周期和验收标准，以 `doc/v2.0-outline-navigation-prd-zh.md` 为准。下方内容仅保留为早期设计摘要，开发时不应作为完整实现依据。
...
该功能已纳入 v2.3 规划，具体产品边界和验收标准以 `doc/v2.3-product-plan-zh.md` 为准。实施时需处理：
